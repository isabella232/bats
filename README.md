...
```
cd bats
mkvirtualenv --no-site-packages bats
...
```
* * * * * ubuntu bash /home/ubuntu/apps/bats/repository/cron.sh fab $DEPLOYMENT_TARGET cron_jobs.test 
```
...
|--------|------|-----|-----|--------|--------|
|bats|tweet|`location`||||
|bats|facebook|`location`||||
|bats|email|`location`||||
|bats|new-comment||||
|bats|open-share-discuss||||
|bats|close-share-discuss||||
|bats|summary-copied||||
|bats|featured-tweet-action|`action`||``tweet_url``|
|bats|featured-facebook-action|`action`||``post_url``|
